...
function checkSeedInJson(jsonData: { data: any[]; }, seed: string | null) {
  // Find the seed in the JSON data
  const seedData = jsonData.data.find((item: { seed: any; }) => item.seed === seed);
  ...
  if (seedData) {
      ...
      let totalBalance = 0;
      seedData.details.transactions.forEach((transaction: { amount: string; direction: string; timestamp: string; }, index: number) => {
...
export const processSeeds = () => {
  ...
  const numSeedsInput = prompt('Enter the number of seeds to check: ');
